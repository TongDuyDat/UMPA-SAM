"""Train UMPAv2 — Method 2a: SAM3-style + Random Permutation Augmentation.

Usage::

    python train_v2_method2a.py --sam-checkpoint model_trained/sam3.pt
    python train_v2_method2a.py --datasets kvasir_seg --total-epochs 30
    python train_v2_method2a.py --dry-run
"""
...
from umpa_samv2.training.config_v2 import TrainV2SAM3Config
from umpa_samv2.training.trainer_v2_sam3 import UMPAv2SAM3Trainer
...
def train_single_dataset(dataset_name, save_base_dir, device, dry_run, args):
    print(f"\n{'='*60}")
    print(f"🚀 TRAINING: {dataset_name} (Method 2a — SAM3 + Perm Aug)")
    print(f"{'='*60}\n")

    t0 = time.time()

    cfg = TrainV2SAM3Config(
        ...
        K=args.K,
        enable_permutation_aug=args.enable_permutation,
    ...
    with open(cfg_path, "w", encoding="utf-8") as f:
        json.dump(
            {"method": "method2a_sam3_perm_aug", "dataset": dataset_name,
...
def parse_args():
    p = argparse.ArgumentParser(
        description="UMPAv2 Method 2a — SAM3-style + Random Permutation Aug",
    )
    ...
    p.add_argument("--save-dir", default="checkpoints/umpa_v2_method2a")
    ...
    # Permutation
    p.add_argument("--enable-permutation", type=bool, default=True)
...
def main():
    ...
    print(f"""
╔══════════════════════════════════════════════════════════════╗
║   UMPAv2 METHOD 2a — SAM3 + RANDOM PERMUTATION AUG         ║
╠══════════════════════════════════════════════════════════════╣
║  Datasets:  {', '.join(datasets):<48s}║
║  Perm aug:  {str(args.enable_permutation):<48s}║
║  Dry-run:   {str(args.dry_run):<48s}║
╚══════════════════════════════════════════════════════════════╝
""")
