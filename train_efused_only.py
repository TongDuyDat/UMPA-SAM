"""Train UMPA-SAM with ONLY E_fused (no E_sparse in mask decoder input).

Architecture:
    Standard:      sparse_prompt_embs = cat([E_sparse, E_fused])
    This script:   sparse_prompt_embs = E_fused.unsqueeze(1)

Runs sequentially on all 5 polyp datasets with full logging.
Error-resilient: saves emergency checkpoint on crash.

Usage:
    python train_efused_only.py
    python train_efused_only.py --dry-run
    python train_efused_only.py --dataset kvasir_seg
"""
...
# ======================================================================
# Model: E_fused only (override forward to skip E_sparse concat)
# ======================================================================

class EFusedOnlyModel(UMPAModel):
    """UMPAModel variant: Mask Decoder receives ONLY E_fused.

    Standard:   sparse_prompt_embeddings = cat([E_sparse, E_fused.unsqueeze(1)])
    This:       sparse_prompt_embeddings = E_fused.unsqueeze(1)
    """
    ...
    def forward(
        ...
        """Forward pass: only E_fused goes to mask decoder."""
        # --- Perturbation ---
        ...
        # ============================================================
        # KEY: Only E_fused, NO E_sparse
        # Standard: cat([sparse_embs, e_fused.unsqueeze(1)])
        # Here:     e_fused.unsqueeze(1) only
        # ============================================================
...
class FaultTolerantTrainer:
    ...
    def _init_log_file(self):
        with open(self.log_file_path, "w", encoding="utf-8") as f:
            f.write(f"{'='*60}\n")
            f.write(f"UMPA-SAM Training — E_fused Only\n")
    ...
    def run(self):
        ...
        if os.path.exists(best_path):
            ...
            with open(results_path, "w", encoding="utf-8") as f:
                json.dump({"dataset": self.dataset_name, "variant": "efused_only", **final}, f, indent=2)
...
def build_model(sam_checkpoint, device):
    ...
    # Build base UMPAModel then swap class to EFusedOnlyModel
    model = UMPAModel.from_config(model_config=model_config)
    model.__class__ = EFusedOnlyModel
...
def train_one_dataset(dataset_name, args):
    ...
    print(f"\n{'█'*60}")
    print(f"█ E_FUSED ONLY — Dataset: {dataset_name}")
    print(f"{'█'*60}")
...
def main():
    parser = argparse.ArgumentParser(description="UMPA-SAM: Train with E_fused ONLY (5 datasets)")
    parser.add_argument("--dataset", type=str, choices=list_datasets(), help="Only this dataset")
    parser.add_argument("--save-dir", type=str, default="checkpoints/efused_only")
    ...
    print(f"\n{'#'*60}")
    print(f"# UMPA-SAM — E_fused Only Training")
