...
class PolypBenchmarkEvaluator:
    ...
    def from_checkpoint(
        ...
        **dataset_kwargs,
        ...
        img_size = getattr(model_config, "image_size", 1008)
        if dataset_cfg.transform_cfg.image_size != img_size:
            from dataclasses import replace as _replace
            dataset_cfg = _replace(
                dataset_cfg,
                transform_cfg=_replace(dataset_cfg.transform_cfg, image_size=img_size),
            )
    ...
    def evaluate(self) -> Dict[str, float]:
        ...
        with torch.inference_mode():
            ...
            for batch in pbar:
                ...
                expected_size = getattr(self.model, "image_size", 1008)
                if images.shape[-1] != expected_size or images.shape[-2] != expected_size:
